...
void CliParser::init_cli() noexcept
{
    ...
    app_.add_option("--repo", repo_path_, "repository_path");
    ...
    path_cmd->add_option("-a,--add", path_add_)->expected(1, 99);
    path_cmd->add_option("-r,--remove", path_remove_)->expected(1, 99);
    path_cmd->add_option("-m,--modify", path_modify_)->expected(1);

    app_.add_option("path_or_tag", tag_or_path_);
    app_.add_flag("-r", remove_flag_, "Remove tag/path association");
    app_.add_flag("-c,--create", create_flag_);
    app_.add_option("-t,--tag", tags_)->expected(1, 99);
    app_.add_option("-p,--path", paths_)->expected(1, 99);
